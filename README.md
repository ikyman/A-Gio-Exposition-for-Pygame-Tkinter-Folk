...
If, by some odd sequence of events, you, despite being on LinkdIn, are a non-coder, your feedback is prized above all. I decree that this Tutorial should be Suis Generus (E.G. Je ne sans pas) be a jolly good read. Something one would conceivably read even without the urges of GUI-making bearing down upon them. Thus, I would very much any non-programmers feedback on the per-se entertainment value of this informative tutorial
...
Then we hop over to pygame, to view a different method of writing a GUI in python. Not only is the language the same, but the skeleton of the GUI application is similar.
